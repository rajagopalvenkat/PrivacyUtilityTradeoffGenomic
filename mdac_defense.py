...
f=open("flipcounts/mix_def_results/ada/mdac_k_"+str(k)+'_w_'+str(mask_wt)+"_"+str(sys.argv[3]), 'w')
...
while uncovered.sum()>0:
	...
	for j, dv in zip(indices_mod, dec_vars_mod):
		if dv==1 and flipped_dv[j]==0 and masked_dv[j]==0 and dnf[j]==0:
			flipped_dv[j]=1
			fc+=1
			score_i+=Delt_flip[:,j]
			...
			if j in LD_df['keys'].unique():
				...
				for v in vs:
					if flipped_dv[v]==0 and masked_dv[v]==0 and dnf[v]==0:
						flipped_dv[v]=1
						fc+=1
						score_i+=Delt_flip[:,v]
		...
		if dv==0 and flipped_dv[j]==0 and masked_dv[j]==0 and dnf[j]==0:
			masked_dv[j]=1
			mc+=1
			score_i+=Delt_mask[:,j]
			Delt_flip[:,j]=-float('inf')
...
dvf = open("dvar_mdac_ada_"+str(k)+"_"+str(mask_wt)+"_"+str(sys.argv[3]), 'w')
